  ...
  <span class="sml"><strong>{{ $client->name }}</strong></span><br>
    ...
    <span class="sml">{{ $client->country }}  |  {{ $client->email }}</span>
      	<div class="pull-right">
      	  <i class="fa fa-clock-o"></i><span class="big">
           {{ $total_time }}</span> hour worked &nbsp;&nbsp;&nbsp;
		</div>
      	<div class="line"></div>
			...
			<h3> Projects</h3>
      	  	<table class="table">
                  ...
                  <th>Status</th>
      		  		...
      		  		<tbody>
      		  		<?php
                //print_r($activity_hours);
                //die();
                 $count=0;
                 $i=0;
                 foreach ($projects as $key => $project) 
                 { 
                  if($activity_hours['tTime'][$i]<1)
                  {
                    $activity_hours['tTime'][$i]=0;
                  }
                ?>
                <tr id="taskItem">
                  <td>{{ ++$count }}</td>
                  <td><a href="#" class="blackLink"><?= $project->prj_title; ?></a></td>
                  <td><?= $activity_hours['tTime'][$i]; ?> Hrs</td>
                  <td>80%</td>
